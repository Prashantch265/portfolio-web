...
window.BlueprintDiagram = (function () {
  ...
  function renderSVG(host, data, panel) {
    ...
    if (!window.matchMedia || !("IntersectionObserver" in window)) {
      ...
      // actually in the viewport, paused otherwise. Also reports
      // in/out to the shared ambient-motion coordinator (app.js,
      // PRD §6.3) so the diagram's pulse takes precedence over the
      // status strip's if both are ever in view at once.
        ...
        var pulseIO = new IntersectionObserver(
          function (entries) {
            entries.forEach(function (entry) {
              ...
              if (window.SchematicAmbientMotion) {
                window.SchematicAmbientMotion.setDiagramActive(entry.isIntersecting);
              }
            });
          },
          { threshold: 0 }
