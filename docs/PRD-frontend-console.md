...
**Sibling documents:** `PRD-frontend-blueprint.md` — the currently-chosen "Blueprint" direction (swiss grid, hairline rules, diagrams-as-hero-content). This document is a full alternate direction, **"Console,"** presented for side-by-side comparison. Also see `PRD-frontend-ledger.md` ("Ledger," editorial-technical alternate) and `PRD-frontend-schematic.md` ("Schematic," a fourth alternate that grafts this document's command palette and status strip onto Blueprint's chassis). None of these are adopted; nothing in `PRD-frontend-blueprint.md`, `PRD-backend.md`, or any built mockup changes as a result of this document existing.
