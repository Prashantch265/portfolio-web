...
**Sibling documents:** `PRD-frontend-blueprint.md` (currently-chosen "Blueprint" direction), `PRD-frontend-console.md` (alternate "Console" direction), and `PRD-frontend-schematic.md` (alternate "Schematic" direction — a fourth option grafting Console's palette/status-strip onto Blueprint's chassis). This document is a third alternate, **"Ledger,"** presented for side-by-side comparison. It is not adopted; nothing in the other documents or their built mockups changes as a result of this document existing.
